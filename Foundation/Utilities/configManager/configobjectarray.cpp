    ...
    : ConfigBase(ConfigElementInfo::ConfigObjArr, parent), configObjClassName(configObjClassName)
{
    ...
    {
        qFatal("You should call ConfigObject::init() in the constructor of class %s",
               configObjClassName.toStdString().c_str());
        return;
    }
    ...
    for (int i = 0; i < configNamesToShow.count(); i++)
    {
        ...
        if (elementType == ConfigElementInfo::ConfigArr || elementType == ConfigElementInfo::ConfigObj
...
int ConfigObjectArray::configCount() const
{
    return count();
}
...
bool ConfigObjectArray::read(const QJsonValue &jsonValue)
{
    ...
    //                configObj->connectConfigChangedSignalToSlot(configChangedSignalReceiver,
    //                configChangedReceiverSlotIndex, false);
    //            }
    //        }
    //    }
    ...
    for (int i = 0; i < jsonArr.count(); i++)
    {
        ...
        else
        {
            ConfigObject *configObj
                = qobject_cast<ConfigObject *>(QObjectFactory::createObject(configObjClassName, this));
            ...
            if (configChangedSignalReceiver != nullptr)
            {
                configObj->connectConfigChangedSignalToSlot(configChangedSignalReceiver, configChangedReceiverSlotIndex,
                                                            true);
            }
